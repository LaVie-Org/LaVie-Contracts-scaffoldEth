...
contract Accounts is ERC721, ERC721URIStorage, ContextMixin, Ownable {
    ...
    constructor(Items _items)  ERC721("LaVAccounts", "LaVAccounts") public {
    ...
    function createAccount(address player, string memory playerTokenURI, uint256 accountType) external onlyOwner returns (uint256) {
        _tokenIds.increment();
    ...
    function initAccountItems(address player, uint256 accountId, uint256 accountType) private onlyOwner {
        if(accountType == 3){
    ...
    function transferItemsFromGameToAccount(address player, uint256 accountId, uint256[] memory ids, uint256[] memory amounts) private onlyOwner {
        itemsContract.transferBatchFromGame(address(itemsContract), player, ids, amounts , "0x0");
    ...
    function changeAccountOwnership(address oldPlayer, uint256 accountId, address newPlayer) private {
        uint256[] memory itemCount = getItemAmounts(oldPlayer, accountId);
        itemsContract.transferBatchFromGame(oldPlayer, newPlayer, account[accountId][oldPlayer].items, itemCount, "0x0");
        
        account[accountId][newPlayer].owner = newPlayer;
        account[accountId][newPlayer].accountId = account[accountId][oldPlayer].accountId;
        account[accountId][newPlayer].items = account[accountId][oldPlayer].items;
        account[accountId][newPlayer].deleted = account[accountId][oldPlayer].deleted;

        uint256[] memory ids = account[accountId][newPlayer].items;
        for (uint256 i = 0; i < ids.length; ++i) {
            account[accountId][newPlayer].inventory[ids[i]].amount =  account[accountId][oldPlayer].inventory[ids[i]].amount;
        }
    ...
    function deleteAccountItems(address player, uint256 accountId) private onlyOwner {
        uint256[] memory itemCount = getItemAmounts(player, accountId);
        itemsContract.burnBatch(player, account[accountId][player].items, itemCount);
    }

    function playerReceivesItemFromGame(address player, uint256 accountId, uint256 itemId) public onlyOwner {
        itemsContract.transferFromGame(address(itemsContract), player, itemId, 1, "0x0");
        addItemToAccount(player, accountId, itemId, 1);
    }

    function playerReceivesMultItemFromGame(address player, uint256 accountId, uint256[] memory itemIds) public onlyOwner {
        for(uint256 i=0; i < itemIds.length; i++){
            playerReceivesItemFromGame(player, accountId, itemIds[i]);
        }
    }

    function playerReceivesItemFromMarket(address fromPlayer, address toPlayer, uint256 itemId, uint256 amount) public {
        require(_msgSender() == address(itemsContract), 'La Vie: Function callable only on trade.');
    ...
    function playerReceivesMultItemFromMarket(address fromPlayer, address toPlayer, uint256[] memory itemIds, uint256[] memory amounts) public {
        require(_msgSender() == address(itemsContract), 'La Vie: Function callable only on trade.');
        for(uint256 i=0; i < itemIds.length; i++){
            playerReceivesItemFromMarket(fromPlayer, toPlayer, itemIds[i], amounts[i]);
        }
    }

    function addItemToAccount(address player, uint256 accountId, uint256 itemId, uint256 amount) internal {
    ...
    function deleteItemFromAccount(address player, uint256 accountId, uint256 itemId, uint256 amount) internal {
        ...
        for(uint256 i = 0; i < arrLength; i++){
            if(account[accountId][player].items[i] == itemId){
                if(i == arrLength - 1) {
                    ...
                    account[accountId][player].items[i] = account[accountId][player].items[arrLength - 1];
    ...
    function getItemAmounts(address player, uint256 accountId) private view returns(uint256[] memory) {
    ...
    function itemExists(uint256 itemId) public returns(bool) {
        return itemsContract.exists(itemId);
    }
    
    function isApprovedForAll(address _owner, address _operator) public override view returns (bool isOperator) {
      // if OpenSea's ERC721 Proxy Address is detected, auto-return true
        if (_operator == address(MARKETPLACE_OPERATOR) || _operator == address(ACCOUNT_MANAGER)) {
            return true;
        }
